...
class GestaoController:
    ...
    def _trans_to_dict(self, trans):
        ...
        category_name = (
            trans.categoria_rel.categoria if trans.categoria_rel else "Sem Categoria"
        )
        classificacao_name = (
            trans.classificacao_rel.classificacao
            if trans.classificacao_rel
            else "Sem Classificação"
        )

        # Safe date formatting
        data_divida_str = (
            trans.data_debito.strftime("%Y-%m-%d") if trans.data_debito else ""
        )
        data_prevista_str = (
            trans.data_prevista.strftime("%Y-%m-%d") if trans.data_prevista else ""
        )
        data_entrada_str = (
            trans.data_entrada.strftime("%Y-%m-%d") if trans.data_entrada else ""
        )
        ...
        return {
            ...
            "data": (
                data_divida_str if type_str == "DEBT" else data_entrada_str
            ),  # Legacy/Compat field?
            "type": type_str,
    ...
    def get_dividas(self, search_term=""):
        ...
        return [
            d
            for d in results
            if search in d["cpf"].lower()
            or search in d["nome"].lower()
            or search in d["categoria"].lower()
            or search in d["classificacao"].lower()
        ]
    ...
    def get_entradas(self, search_term=""):
        ...
        return [
            p
            for p in results
            if search in p["cpf"].lower()
            or search in p["nome"].lower()
    ...
    def criar_recibo(self, nome, cpf, valor):
        """Generates a receipt graphical control."""
        return criar_recibo(nome, cpf, valor)

    def download_recibo_obj(self, nome, cpf, valor, categoria, data_transacao):
        """
        Gera o PDF, salva em storage/temp e copia para assets/temp para download.
        Retorna a URL relativa para abrir no navegador.
        """
        from controllers.geral_controller import gerar_pdf_bytes

        # 1. Gerar Bytes
        pdf_bytes = gerar_pdf_bytes(nome, cpf, valor, categoria, data_transacao)

        # 2. Definir caminhos
        # Base do projeto (assumindo execution from root or src)
        # Se rodando de src/main.py, cwd geralmente é root se chamado como python src/main.py
        # Vamos usar caminhos absolutos baseados no arquivo atual

        base_src = os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))
        )  # .../src
        project_root = os.path.dirname(base_src)  # .../Counts2

        # Storage Path (Persistent)
        storage_dir = os.path.join(project_root, "storage", "temp")
        os.makedirs(storage_dir, exist_ok=True)

        # Assets Path (Public/Serving)
        assets_temp_dir = os.path.join(base_src, "assets", "temp")
        os.makedirs(assets_temp_dir, exist_ok=True)

        # Filename unique
        filename = f"recibo_{int(time.time())}.pdf"

        storage_path = os.path.join(storage_dir, filename)
        assets_path = os.path.join(assets_temp_dir, filename)

        # 3. Salvar
        with open(storage_path, "wb") as f:
            f.write(pdf_bytes)

        # 4. Copiar para assets para servir
        shutil.copy(storage_path, assets_path)

        # 5. Retornar URL relativa
        # Flet serve assets_dir na raiz "/"
