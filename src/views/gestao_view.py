...
class GestaoView(ft.Column):
    ...
    def _build_usuarios_tab(self):
        ...
        return ft.Container(
            content=ft.Column(
                [bt_new_user, ft.Container(height=20), self.users_column], expand=True
            ),  # Expand outer column so scrolling works if needed
            padding=20,
            expand=True,
        )
    ...
    def _build_dividas_tab(self):
        ...
        bt_new_divida = ft.Row(
            [
                ft.ElevatedButton(
                    ...
                    width=100,
                    color=ft.Colors.WHITE,
                    on_click=lambda e: self._show_action_dialog(
                        "nova_divida_tab", None
                    ),
                ),
                ft.Container(width=20),
        ...
        return ft.Container(
            content=ft.Column(
                [bt_new_divida, ft.Container(height=20), self.dividas_column],
                expand=True,
            ),
            padding=20,
            expand=True,
    ...
    def _build_entradas_tab(self):
        ...
        self.entradas_column = ft.Column(
            spacing=10, scroll=ft.ScrollMode.ADAPTIVE, expand=True
        )
        ...
        return ft.Container(
            content=ft.Column(
                [bt_new_entrada, ft.Container(height=20), self.entradas_column],
                expand=True,
            ),
            padding=20,
            expand=True,
    ...
    def _deprecated_build_relatorios_tab(self):
        ...
        user_opts = [ft.dropdown.Option(key="", text="Todos")] + [
            ft.dropdown.Option(key=u["cpf"], text=f"{u['nome']} ({u['cpf']})")
            for u in users
        ]
        ...
        cat_opts = [ft.dropdown.Option(key="", text="Todas")] + [
            ft.dropdown.Option(key=c["categoria"], text=c["categoria"])
            for c in categorias
        ]

        self.filter_categoria = ft.Dropdown(
            label="Categoria", width=250, options=cat_opts, value=""
        )
        ...
        return ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Column(
                        [self.report_table], scroll=ft.ScrollMode.AUTO, height=400
                    ),
                ],
    ...
    def _build_relatorios_tab(self):
        ...
        self.filter_categoria = ft.Dropdown(
            label="Categoria", options=cat_opts, value="", col={"xs": 12, "md": 6}
        )
    ...
    def _show_filter_dialog(self):
        ...
        content = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.ResponsiveRow(
                        [self.filter_categoria, self.filter_tipo], run_spacing=10
                    ),
                    ft.ResponsiveRow(
                        [self.filter_data_inicial, self.filter_data_final],
                        run_spacing=10,
                    ),
                ],
                tight=True,
        ...
        self.filter_dialog = ft.AlertDialog(
            ...
            actions=[
                ...
                ft.TextButton(
                    "Cancelar", on_click=lambda e: self.page.close(self.filter_dialog)
                ),
            ],
            modal=True,
                ...
                [
                    ...
                    ft.Column(
                        [
                            ft.Container(
                                content=ft.Text(
                                    t["categoria"], size=12, color=ft.Colors.BLACK87
                                ),
        ...
        return ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        value, size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE
                    ),
                ]
            ),
            bgcolor=color,
            ...
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ...
                            ft.Column(
                                [
                                    ft.Text(
                                        u["nome"], weight=ft.FontWeight.BOLD, size=16
                                    ),
                                    ft.Text(
                                        f"CPF: {u['cpf']}",
                                        size=12,
                    ...
                    ft.Row(
                        [
                            ft.Text(
                                f"Maior Pago: R$ {u['maior_pago']:.2f}",
                                size=12,
                                color=ft.Colors.GREY,
                            ),
                            ft.Container(expand=True),
                            ...
                            ft.IconButton(
                                ft.Icons.DELETE,
                                icon_color=bt_del_atributos["cor"],
                                tooltip=bt_del_atributos["tooltip"],
                                on_click=lambda e: self._show_action_dialog(
                                    "deletar usuario", u
                                ),
                                disabled=bt_del_atributos["inativo"],
                                icon_size=def_icon_size,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.END,
    ...
    def _build_transaction_card(self, d: dict, type_t: str):
        ...
        info_classificacao = d.get("classificacao")
        bt_quitar = ft.IconButton(
            ft.Icons.ATTACH_MONEY,
            icon_color=ft.Colors.GREEN_300,
            tooltip=f"Quitar {info_classificacao}",
            on_click=lambda e: self._show_action_dialog("quitar_transacao", d, type_t),
            disabled=False,
            icon_size=def_icon_size,
        )

        bt_recibo = ft.IconButton(
            ft.Icons.RECEIPT,
            icon_color=ft.Colors.BLUE_300,
            tooltip="Recibo",
            on_click=lambda e: self._show_action_dialog("recibo_transacao", d, type_t),
            disabled=False,
            icon_size=def_icon_size,
        )
        ...
        data_prevista = ft.Row(
            [
                ...
                ft.Text(
                    f"previsao: {d['data_prevista']}", size=12, color=ft.Colors.GREY
                ),
            ],
            alignment=ft.MainAxisAlignment.START,
        )
    ...
    def update_usuarios_table(self):
        ...
        for u in users:
            if search and (
                search not in u["cpf"].lower() and search not in u["nome"].lower()
            ):
                continue
    ...
    def update_dividas_table(self):
        ...
        data = self.controller.get_dividas(
            self.search_dividas.value if hasattr(self, "search_dividas") else ""
        )
        self.dividas_column.controls.clear()

        for d in data:
            self.dividas_column.controls.append(
    ...
    def update_entradas_table(self):
        ...
        data = self.controller.get_entradas(
            self.search_entradas.value if hasattr(self, "search_entradas") else ""
        )
        self.entradas_column.controls.clear()

        for d in data:
            self.entradas_column.controls.append(
    ...
    def update_reports(self):
        ...
        if hasattr(self, "filter_categoria") and self.filter_categoria.value:
            categoria = (
                ...
                if self.filter_categoria.value != ""
                else None
            )
        ...
        if hasattr(self, "filter_data_inicial") and self.filter_data_inicial.value:
            try:
                data_inicial = (
                    dt.strptime(self.filter_data_inicial.value, "%Y-%m-%d").date()
                    if self.filter_data_inicial.value
                    else None
                )
            except ValueError:
                data_inicial = None
        ...
        if hasattr(self, "filter_data_final") and self.filter_data_final.value:
            try:
                data_final = (
                    dt.strptime(self.filter_data_final.value, "%Y-%m-%d").date()
                    if self.filter_data_final.value
                    else None
                )
            except ValueError:
                data_final = None
        ...
        self.metrics_container.controls = [
            self._build_metric_card(
                "Total Dívidas", f"R$ {metrics['total_dividas']:.2f}", ft.Colors.RED_400
            ),
            self._build_metric_card(
                "Total Entradas",
            ...
            self._build_metric_card(
                "Maior Dívida",
                f"R$ {metrics['maior_divida']:.2f}",
                ft.Colors.ORANGE_400,
            ),
            self._build_metric_card(
                "Maior Entrada",
                f"R$ {metrics['maior_entrada']:.2f}",
                ft.Colors.BLUE_400,
            ),
        ]
    ...
    def _show_action_dialog(self, action, user_data, transaction_type=None):
        ...
        if action == "novo":
            ...
            categoria = user_data.get("categoria", "Associação")
            data = user_data.get("data", "")  # Data no formato YYYY-MM-DD
            ...
            actions = [
                ...
                ft.ElevatedButton(
                    ...
                    color=ft.Colors.WHITE,
                    on_click=lambda e: self._confirm_delete_user(user_data["cpf"]),
                ),
            ]
            ...
            actions = [
                ...
                ft.ElevatedButton(
                    ...
                    bgcolor=ft.Colors.RED,
                    color=ft.Colors.WHITE,
                    on_click=lambda e: self._confirm_delete_transaction(
                        user_data["id"]
                    ),
                ),
            ]
            ...
            return
        elif action == "quitar_transacao":
            self._build_quitar_dialog(user_data)
            return
    ...
    def _build_new_user_dialog_content(self, user_data=None, transaction_type=None):
        ...
        title_text = (
            ...
            if user_data
                ...
                ("Nova Dívida" if transaction_type == "divida" else "Nova Entrada")
                if transaction_type
                else "Novo Usuário"
            )
        )
        ...
        self.nu_nome = ft.TextField(
            label="Nome", col={"xs": 12, "sm": 12, "md": 12, "lg": 6}
        )
        ...
        if not hasattr(self, "date_picker_prevista"):
            self.date_picker_prevista = ft.DatePicker(
                on_change=lambda e: setattr(
                    self.nu_data_prevista, "value", e.control.value.strftime("%Y-%m-%d")
                )
                or self.page.update(),
                first_date=datetime(2020, 1, 1),
                last_date=datetime(2030, 12, 31),
        ...
        content = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Icon(ft.Icons.PERSON_ADD, size=30, color=ft.Colors.BLUE),
                            ft.Text(
                                title_text,
                                size=20,
                                weight=ft.FontWeight.BOLD,
                    ...
                    ft.ResponsiveRow(
                        [self.nu_categoria, self.nu_valor], run_spacing=10
                    ),
                    ft.Row([toggle_row]),
                    ft.ResponsiveRow(
                        [self.nu_data, self.nu_data_prevista], run_spacing=10
                    ),
                ],
                tight=True,
        ...
        bt_salvar = ft.ElevatedButton(
            ...
            bgcolor=ft.Colors.GREEN,
            color=ft.Colors.WHITE,
            on_click=self._save_new_user,
        )
        bt_limpar = ft.ElevatedButton(
            "Limpar",
            bgcolor=ft.Colors.BLUE,
            color=ft.Colors.WHITE,
            on_click=self._clear_new_user_form,
        )
        bt_cancelar = ft.TextButton("Cancelar", on_click=lambda e: self._close_dialog())

        actions = [bt_salvar, bt_limpar, bt_cancelar]
    ...
    def _build_edit_user_dialog_content(self, user_data):
        ...
        self.eu_nome = ft.TextField(
            label="Nome",
            value=user_data["nome"],
            col={"xs": 12, "sm": 12, "md": 12, "lg": 6},
        )
        self.eu_senha = ft.TextField(
            label="Senha", password=True, can_reveal_password=True, col={"xs": 12}
        )
        ...
        content = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Icon(ft.Icons.EDIT, size=30, color=ft.Colors.BLUE),
                            ft.Text(
                                "Edição de Usuário",
                                size=20,
                                weight=ft.FontWeight.BOLD,
        ...
        actions = [
            ft.ElevatedButton(
                ...
                bgcolor=ft.Colors.GREEN,
                color=ft.Colors.WHITE,
                on_click=self._save_edit_user,
            ),
            ft.ElevatedButton(
                "Limpar",
                bgcolor=ft.Colors.BLUE,
                color=ft.Colors.WHITE,
                on_click=self._clear_edit_user_form,
            ),
            ft.ElevatedButton(
                "Cancelar",
                bgcolor=ft.Colors.ORANGE,
    ...
    def _build_edit_transaction_dialog(self, data, transaction_type):
        ...
        self.et_valor = ft.TextField(
            label="Valor",
            prefix_text="R$ ",
            value=str(data.get("valor")),
            keyboard_type=ft.KeyboardType.NUMBER,
            col={"xs": 12, "sm": 12, "md": 12, "lg": 6},
        )
        ...
        self.et_data_label = ft.Text(
            "Data Dívida" if transaction_type == "divida" else "Data Pagamento", size=16
        )
        ...
        if not hasattr(self, "et_date_picker"):
            self.et_date_picker = ft.DatePicker(
                on_change=lambda e: setattr(
                    self.et_data, "value", e.control.value.strftime("%Y-%m-%d")
                )
                or self.page.update(),
                first_date=datetime(2020, 1, 1),
                last_date=datetime(2030, 12, 31),
        ...
        if not hasattr(self, "et_date_picker_prevista"):
            self.et_date_picker_prevista = ft.DatePicker(
                on_change=lambda e: setattr(
                    self.et_data_prevista, "value", e.control.value.strftime("%Y-%m-%d")
                )
                or self.page.update(),
                first_date=datetime(2020, 1, 1),
                last_date=datetime(2030, 12, 31),
        ...
        content = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.ResponsiveRow(
                        [self.et_categoria, self.et_valor], run_spacing=10
                    ),
                    ft.ResponsiveRow(
                        [self.et_data, self.et_data_prevista], run_spacing=10
                    ),
                ],
                tight=True,
                scroll=ft.ScrollMode.ADAPTIVE,
        ...
        actions = [
            ft.ElevatedButton(
                ...
                bgcolor=ft.Colors.GREEN,
                color=ft.Colors.WHITE,
                on_click=self._save_edit_transaction,
            ),
            ft.ElevatedButton(
                "Cancelar",
                bgcolor=ft.Colors.ORANGE,
    ...
    def _build_quitar_dialog(self, divida_data):
        """Builds confirmation dialog for debt settlement."""

        # Data field with DatePicker
        self.quitar_data_pagamento = ft.TextField(
            label="Data do Pagamento",
            value=datetime.now().strftime("%Y-%m-%d"),
            read_only=True,
            suffix=ft.IconButton(
                icon=ft.Icons.CALENDAR_MONTH,
                on_click=lambda e: self.page.open(self.quitar_date_picker),
            ),
            col={"xs": 12},
        )

        # DatePicker instance
        if not hasattr(self, "quitar_date_picker"):
            self.quitar_date_picker = ft.DatePicker(
                on_change=lambda e: setattr(
                    self.quitar_data_pagamento,
                    "value",
            ...
            self.page.overlay.append(self.quitar_date_picker)

        # Store divida data for confirmation
        self.quitar_divida_data = divida_data

        # Confirmation handler
        def confirmar_quitacao(e):
            self.controller.quitar_divida(
                self.quitar_divida_data, self.quitar_data_pagamento.value
            )
            self._close_dialog()
        ...
        content = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Icon(
                                ft.Icons.ATTACH_MONEY, size=30, color=ft.Colors.GREEN
                            ),
                            ft.Text(
                                "Quitar Dívida",
                                size=20,
                    ...
                    ft.Divider(),
                    ft.Text(f"Categoria: {divida_data['categoria']}", size=14),
                    ft.Text(
                        f"Valor: R$ {divida_data['valor']:.2f}",
                        size=14,
                        weight=ft.FontWeight.BOLD,
                    ),
                    ft.Divider(),
                    ft.ResponsiveRow([self.quitar_data_pagamento]),
                ],
                tight=True,
            ),
            padding=20,
            bgcolor=ft.Colors.WHITE,
            border_radius=10,
            width=min(self.page.width - 20, 500) if self.page else 500,
        )
        ...
        self.page.on_keyboard_event = on_key_down
    ...
    def _clear_new_user_form(self, e):
        if (
            self.is_adding_transaction
            and hasattr(self, "nu_original_user_data")
            and self.nu_original_user_data
        ):
    ...
    def _save_new_user(self, e):
        ...
        data = {
            ...
            "data_prevista": (
                self.nu_data_prevista.value if (not self.nu_is_pago.value) else None
            ),
        }
    ...
    def _save_edit_transaction(self, e):
        ...
        data = {
            ...
            "cpf": (
                self.et_user_dropdown.value
                if self.et_user_dropdown.value
                else self.et_original_data["cpf"]
            ),
            ...
            "data_prevista": (
                self.et_data_prevista.value
                if hasattr(self, "et_data_prevista") and self.et_data_prevista.visible
                else None
            ),
        }
    ...
    def _build_ui(self):
        ...
        header = ft.Container(
            content=ft.Row(
                [
                    ...
                    ft.Icon(
                        ft.Icons.ADMIN_PANEL_SETTINGS, size=40, color=ft.Colors.BLUE_600
                    ),
        ...
        self.controls = [header, self.tabs]
